...
func PowerSavingON(cmd []string, conn *ssh.ServerConn, channel ssh.Channel) {
	...
	Issues := YamiDB.EditFeild(cmd[2], "PowerSavingExempt", "1"); if !Issues {
		channel.Write([]byte("Failed to promote user to Admin"))
		return
	} else {
		channel.Write([]byte("\x1b[38;5;11mUser has been promoted to Power Saving correctly\x1b[0m\r\n"))
	}


	for _, s := range Sessions.Sessions {
		if strings.ToLower(s.User.Username) == cmd[2] {
			s.User.PowerSavingExempt = true
			s.Channel.Write([]byte("\x1b[0m\x1b7\x1b[1A\r\x1b[2K\x1b[38;5;2mYou have been promoted to Power Saving\x1b[0m\x1b8"))
		}

...
func TermFXExecuteCommand(name string, conn *ssh.ServerConn, channel ssh.Channel) error {
	...
	DaysLeft := time.Duration(time.Until(time.Unix(Users.PlanExpiry, 0))).Hours()/24
	SecondsLeft := time.Duration(time.Until(time.Unix(Users.PlanExpiry, 0))).Seconds()
	HoursLeft := time.Duration(time.Until(time.Unix(Users.PlanExpiry, 0))).Hours()


	New.RegisterVariable("daysleft", strconv.FormatFloat(DaysLeft, 'f', 2, 64))
	New.RegisterVariable("hoursleft", strconv.FormatFloat(HoursLeft, 'f', 2, 64))
	New.RegisterVariable("secondsleft", strconv.FormatFloat(SecondsLeft, 'f', 2, 64))

	running,_ := YamiDB.GetRunning()

	New.RegisterVariable("motd", functions.Motd)
	New.RegisterVariable("myip", conn.RemoteAddr().String())
	...
	New.RegisterVariable("id", strconv.Itoa(Users.ID))
	New.RegisterVariable("powercompression", ColourizeBoolen(Users.PowerSavingExempt))
	...
	for _, s := range ClientPoly.BetaMapHandler {

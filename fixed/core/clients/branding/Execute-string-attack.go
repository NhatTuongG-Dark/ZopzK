...
func TermFXExecuteAttack(name string, conn *ssh.ServerConn, channel ssh.Channel, ip, port, times, method string) error {
	...
	New.RegisterVariable("date", time.Now().Format("Mon Jan _2 15:04:05 2006"))
	New.RegisterVariable("target", ip)
	New.RegisterVariable("time", times)
	New.RegisterVariable("port", port)
	New.RegisterVariable("method", method)

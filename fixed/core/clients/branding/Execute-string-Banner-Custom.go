...
func TermFXExecuteBannerCustom(name string, conn *ssh.ServerConn, channel ssh.Channel, tag string, replace string) error {
	...
	New.RegisterVariable(tag, replace)
...
func ColourizeBoolen(boolen bool) string {
	if boolen {
